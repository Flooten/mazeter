...
{	
	if (current_sensor_data.distance3 > THRESHOLD_CONTACT_SIDE)
	{
		...
		else if ((max(previous_sensor_data.distance1, previous_sensor_data.distance2) >= THRESHOLD_STOP_DEAD_END) &&
				 (max(current_sensor_data.distance1, current_sensor_data.distance2) < THRESHOLD_STOP_DEAD_END))
		{
			// Fall 2
			pushTurnStack(turn_stack, newTurnNode(RIGHT_TURN));
			makeTurn(LEFT_TURN);
